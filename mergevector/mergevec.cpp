...
void icvAppendVec( CvVecFile &in, CvVecFile &out, int winwidth, int winheight );
// Merge vec files
void icvMergeVecs( char* infoname, const char* outvecname, int width, int height );
...
void icvMergeVecs( char* infoname, const char* outvecname, int width, int height )
{
    ...
    for ( i = 0; i < filenum ; i++ ) {
        ...
        icvAppendVec( invec, outvec, width, height );
        fclose( invec.input );
    }
    fclose( outvec.input );
}
...
int main( int argc, char **argv ) {
    ...
    icvMergeVecs( infoname, outvecname, width, height );
    return 0;
}
